...
class AsignarUSUpdateView(LoginRequiredMixin, ListView):
    ...
    def get_pendientes_clasificados(self,sprint):
        ...
        """Generando fist list por partes"""
        print("hola")
        """first_list_a= self.get_queryset().exclude(second_list_a)
        first_list_p= self.get_queryset().exclude(second_list_p)
        first_list= first_list_a.intersection(first_list_p)"""
        second_list = second_list_a.union(second_list_p)

        #first_list = self.get_queryset().exclude(second_list)
        aux = UserStory.objects.filter(~Q(sprints_asignados = None))
        first_list=aux.filter(proyecto=self.kwargs['pk_proyecto'],
                                                     estado=1,
                                                    sprint=sprint.pk)
        sl = []
